...
public class Server implements Runnable {
	...
	public void run() {
		try {
			...
			while (true) {
				...
				/*
				 * //클라이언트의 IP를 확인해보자. System.out.println("Client IP: "+s.getInetAddress());
				 * System.out.println("Client Port: "+s.getPort());
				 * 
				 * //메세지전송 OutputStream out=s.getOutputStream(); //서버에서 클라이언트 메모리 연결
	...
	public class Client extends Thread {
		 ...
		 * 
		 * 
		 * <======클라이언트 =====> (1)요청을 위해 필요한 데이터를 전송 ex) 로그인 : id, pwd (3)결과값을 받아서 화면을
		 * 출력!
		 */
		/*
		 * (non-Javadoc)
			...
			try {
				...
				while (true) {
					...
					case Function.LOGIN: {
						...
						/*
						 * 
						 * 로그인 방개설 방들어가기 방나가기 ==> 위에 4개만 잘하면 게임프로그램은 아무것도 아니다~
						 */
					}
						break;
					...
					case Function.CHARACTERCHOICE: {
						...
						// (*1*)제일 먼저 접속한 사람들에게 자신이 접속했다는 것을 알린다.
						messageAll(Function.LOGIN + "|" + charvo.getId() + "|" + charvo.getpos());// 접속한 모든 사람에게 로그인을
																									// 알려준다~(테이블에 출력)

						// (*2*)이후에 자신을 접속 시킨다.
						waitList.add(this);
						// (*3*)아이콘 바꿔주기
						messageTo(Function.CHARACTERCHOICE + "|" + charvo.getId() + "|" + charvo.getIcon() + "|"
								+ charvo.getRank());
						...
						for (Room room : roomVc) {
							if (!room.equals(null)) {
								...
								messageTo(Function.MAKEROOM + "|" + room.roomNumber + "|" + room.roomName + "|"
					...
					case Function.MAKEROOM: {
						// out.write((Function.MAKEROOM + "|" + rname + "|" + state.trim() + "|" +
						// pwd.trim() + "|" + (inwon + 4) + "\n")
						// Room.java = public Room(int roomNumber,String roomName, String roomState,
						// String roomPwd, int maxcount)
						Room room = new Room((roomN++), st.nextToken(), st.nextToken(), st.nextToken(),
								Integer.parseInt(st.nextToken()));
						// room.userVc.addElement(this);
						room.userVc.addElement(this);
						pos = room.roomName;
						roomVc.addElement(room);
						messageAll(Function.MAKEROOM + "|" + room.roomNumber + "|" + room.roomName + "|"
								+ room.roomState + "|" + room.current + "/" + room.maxcount + "|" + room.roomPwd);
						// 2/6
						// 명령(방들어가기)
						System.out.println("MAKEROOM :" + room.current);
						messageTo(Function.MYROOMIN + "|" + charvo.getId() + "|" + charvo.getRank() + "|"
								+ charvo.getIcon() + "|" + room.maxcount + "|" + room.roomNumber);

						// 출력 ==> client
						messageAll(Function.ROOMNAMEUPDATE + "|" + charvo.getId() + "|" + pos + "방");
					}
						break;
					/*
					 * 방찾는다 현재인원 증가 위치 변경 ========== 방에 있는 사람 => 방에 들어가는 사람의 정보 전송 => 입장메세지 방에 들어가는
					 * 사람 처리 => 방으로 변경 => 방에 있는 사람의 모든 정보를 받는다 대기실 처리 => 1) 인원 (table1) 2) 위치
					 * (table2)
					 * 
					 * 강퇴 , 초대 , 게임
					 */
							...
							if (rn == room.roomNumber) {
								...
								if (room.gameState == false) {
									...
									if (room.current >= 3) {
										...
										user.messageTo(Function.GAMEMYMUNJE + "|" + room.goal);
								...
								for (int j = 0; j < room.userVc.size(); j++) {
									...
									user.messageTo(Function.ROOMIN + "|" + charvo.getId() + "|" + charvo.getRank() + "|"
											+ charvo.getIcon() + "|" + room.maxcount);
									user.messageTo(Function.ROOMCHAT + "|[알림 ☞]" + user.charvo.getId() + "님이 입장하셨습니다");
								}
								// 방에 들어가는 사람 처리
								// room.userVc.addElement(this);
								...
								for (int k = 0; k < room.userVc.size(); k++) {
									...
									if (!charvo.getId().equals(user.charvo.getId())) {
										...
										messageTo(Function.ROOMIN + "|" + user.charvo.getId() + "|"
												+ user.charvo.getRank() + "|" + user.charvo.getIcon() + "|"
												+ room.maxcount);
									}
								}
								room.userVc.addElement(this);
								// 대기실
								/*
								 * messageAll(Function.WAITUPDATE+"|" +id+"|"+pos+"|"+room.roomName+"|"
						...
						for (int i = 0; i < roomVc.size(); i++) {
							...
							if (rn == room.roomNumber) {
								...
								for (int j = 0; j < room.userVc.size(); j++) {
									Client user = room.userVc.elementAt(j);
									user.messageTo(
											Function.ROOMIN + "|" + user.charvo.getId() + "|" + user.charvo.getRank()
													+ "|" + user.charvo.getIcon() + "|" + room.maxcount);
									user.messageTo(Function.ROOMCHAT + "|[알림 ☞]" + user.charvo.getId() + "님이 입장하셨습니다");
									// user.messageTo(Function.ROOMCHAT+
									// "|"+user.charvo.getId());
								}
								...
								room.userVc.addElement(this);
								// 대기실
								messageAll(Function.WAITUPDATE + "|" + charvo.getId() + "|" + pos + "|" + room.roomName
										+ "|" + room.current + "|" + room.maxcount);
							}
						}
					}break;
						...
						for (int i = 0; i < roomVc.size(); i++) {
							...
							if (rn == room.roomNumber) {
								...
								for (int j = 0; j < room.userVc.size(); j++) {
									...
									user.messageTo(Function.ROOMCHAT + "|[알림 ☞]" + charvo.getId() + "님이 퇴장하셨습니다");
								}
								// 방에 있는 사람 처리??
								// room.userVc.addElement(this);
								...
								for (int k = 0; k < room.userVc.size(); k++) {
									...
									if (charvo.getId().equals(user.charvo.getId())) {
										...
										for (int z = 0; z < room.userVc.size(); z++) {
											System.out.println(
													(z + 1) + "번째: " + room.userVc.elementAt(z).charvo.getId());
										}
										break;
									}
								}
					...
					case Function.GAMESTART: {
						...
						for (int i = 0; i < roomVc.size(); i++) {
							...
							if (Integer.parseInt(rn) == room.roomNumber) {
								...
								user.messageTo(Function.GAMESTART + "|" + room.goal);
								
								for (int k = 1; k < room.userVc.size(); k++) {
									user = room.userVc.get(k);
									//get(i)=elements									
									user.messageTo(Function.ROOMCHAT + "|" + "[안내]게임이 시작됩니다.");
									user.messageTo(Function.ROOMCHAT + "|" + round+"라운드를 실행합니다.!");
									user.messageTo(Function.GAMESTART + "|" + room.goal);
									user.messageTo(Function.REMOVEACTIONLISTENER + "|");
								}
							}
						}
					} // swith문 끝
					...
					case Function.GAMEYESNO: {
						...
						for (int i = 0; i < roomVc.size(); i++) {
							Room room = roomVc.elementAt(i);
							if (Integer.parseInt(rn) == room.roomNumber) {
								System.out.println("방 번호 비교 완료:"+room.roomNumber);
								System.out.println("조건 비교.:" +"넘어온 msg, data : "+data + "   GOAL : "+room.goal);
								if(room.goal.equals(data))
								{
						...
						for (int i = 0; i < roomVc.size(); i++) {
							...
							if (Integer.parseInt(rn) == room.roomNumber) {
								for (int k = 0; k < room.userVc.size(); k++) {
									Client user = room.userVc.get(k);
									// get(i)=elements
									user.messageTo(Function.GAMECLEANALL + "|");
								}
							}
						}
					}break;
